...
function scene:create( event )
	...
	--냄비
	local pot = display.newImage("이미지/음식/재료/1차/당근/5물+당근냄비_끓는.png")
	pot.x = display.contentWidth * 0.363
	pot.y = display.contentHeight * 0.4
	sceneGroup:insert(pot)

	--주걱
	local spoon = display.newImageRect("이미지/음식/재료/1차/당근/주걱.png",126.5, 497.5)
	spoon.x = display.contentWidth *0.85
			...
			if(event.target.isFocus) then
				...
				event.target.y < pot.y + 100 and event.target.y > pot.y - 100 )then
					display.remove(event.target)
            			...
            			composer.gotoScene("soupView8_1",options)
				else
				end
